  ...
  <div class="home">
    ...
    <div class="swiperWrap center">
      <van-swipe :autoplay="3000" indicator-color="white">
        <template v-if="bannerList.length > 0">
          <div @click="$router.push('/live')" v-for="banner in bannerList" :key="banner.id">
    ...
    <div class="tuijian">
      ...
      <div class="content">
        ...
        <!-- 小图分组 -->
        <div
          class="smallGroup oneLineBetween f-wrap"
          v-if="RecommendList && RecommendList.video.length > 5"
        >
          <!-- item 单个 start -->
          <div
            class="item"
            v-for="(item,key) in RecommendList.video.slice(1,5)"
            :key="key"
            @click="toVideoDetailPage(key)"
          >
            <div class="coverBox">
              <img class="coverImg" :src="filterImg(item.image)" alt>
            </div>
            <div class="rightRightCorner oneLineBetween">
              <div>❤</div>
...
export default {
  ...
  methods: {
    ...
    // async getRecommend(){
    //   const {data:ret} = await this.$http.post(this.setApi + API.recommendList,{type:5})
    //   console.log(ret)
    // },
    // 推荐视频
    getRecommend() {
      this.$http
        ...
        .then(res => {
          ...
          console.log('vvvvvvvvv',this.RecommendList)
        });
    },
    filterImg(img) {
      return $.filterUrl(img);
    },
    toVideoDetailPage(index) {
      localStorage.setItem(
        "videoDetailPage_PlayUrl",
        this.RecommendList.video[index].video
      );
      localStorage.setItem(
        "videoDetailPage_ImgUrl",
        this.RecommendList.video[index].image
      );
      localStorage.setItem(
        "videoDetailPage_Conent",
        this.RecommendList.video[index].content
      );
